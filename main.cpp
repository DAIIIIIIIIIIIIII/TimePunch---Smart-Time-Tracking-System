...
void inserimento_orari (char *nome) {
    ...
    if (file == NULL) {
        ...
        if (file == NULL) {
            ...
            fseek(file, posizione_riga - 1, SEEK_SET);
...
void crea_nuovo_utente() {
    ...
    if (file == NULL) {
        ...
        mkdir(strcat(strcpy(ultima_linea, "orari/"), nome));                            //Creazione della cartella con nome utente
...
int main() {
    ...
    while (true) {
        ...
        scanf_s("%3s", codice, (unsigned)sizeof(codice));                   //evita l'overflow a massimo 3 valori
